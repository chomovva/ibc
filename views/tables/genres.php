...
<div class="wrap">
	...
	<nav class="nav-tab-wrapper wp-clearfix" aria-label="<?php network_admin_url( __( 'Вторичное меню', IBC_TEXTDOMAIN ) ); ?>">
		<a href="<?php $this->the_genre_link( array( 'nonce' => $nonce ) ); ?>" class="nav-tab <?php echo ( 'table' == $action ) ? 'nav-tab-active' : ''; ?>">
			<?php _e( 'Список', IBC_TEXTDOMAIN ); ?>
		</a>
		<a href="<?php $this->the_genre_link( array( 'action' => 'add', 'nonce' => $nonce ) ); ?>" class="nav-tab <?php echo ( 'add' == $action ) ? 'nav-tab-active' : ''; ?>">
	...
	<?php else : ?>
		<?php
			...
			if ( 'edit' == $action && isset( $_REQUEST[ 'id' ] ) ) {
				$genre = $this->get_genre( $_REQUEST[ 'id' ] );
							...
							<div class="col-xs-12 col-sm-6">
								<select style="width: 100%;" name="parent">
									...
									<?php foreach ( $genres as $genre ) : ?>
										<option value="<?php echo esc_attr( $genre->id ); ?>" <?php selected( $genre->id, $parent ); ?> ><?php echo $genre->name ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
